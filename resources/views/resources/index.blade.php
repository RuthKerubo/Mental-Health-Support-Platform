    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <form action="{{ route('resources.index') }}" method="GET" class="flex">
                ...
                <button type="submit" class="px-3 py-1 bg-blue-500 text-white rounded-r-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($resources as $resource)
                            <div class="bg-white dark:bg-gray-700 rounded-lg shadow-md overflow-hidden flex flex-col">
                                ...
                                <div class="p-4 flex-grow">
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-300 mb-4 line-clamp-2">{{ $resource->description }}</p>
                                </div>
                                <div class="px-4 pb-4 mt-auto flex justify-between items-center">
                                    <a href="{{ route('resources.show', $resource) }}" class="text-sm text-blue-500 hover:underline">View Details</a>
                                    @if($resource->file_path)
                                        <a href="{{ asset('storage/' . $resource->file_path) }}" download class="text-sm px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600">
                                            Download
                                        </a>
                                    @endif
                                </div>
